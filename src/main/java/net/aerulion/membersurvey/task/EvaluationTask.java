...
public class EvaluationTask extends BukkitRunnable {
    ...
    private final String[] chatColors = {"§2", "§3", "§4", "§5", "§6", "§7", "§9", "§a", "§b", "§c", "§d", "§f"};
    ...
    public void run() {
        ...
        for (String player : survey.getResults().keySet()) {
            if (results.containsKey(survey.getResults().get(player))) {
                results.put(survey.getResults().get(player), results.get(survey.getResults().get(player)) + 1);
            } else {
        ...
        for (String option : results.keySet()) {
            ...
            for (int i = 1; i < (((double) results.get(option) / (double) survey.getResults().size()) * 160D); i++) {
                sb.append("|");
            }
